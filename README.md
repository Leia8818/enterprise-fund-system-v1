...
1. 打开系统首页，查看 Dashboard 的核心余额和“今日待处理清单”。
2. 进入“资金流水”，系统会显示字段完整/字段缺失数量。
3. 新增或编辑流水时，金额只录入正数；系统按“收入/支出/归还”统一显示正负号。
4. 点击“归档已完成”可批量归档状态为“已完成”的流水。
5. 归档不会删除历史记录；财务统计仍会包含这些流水，日常列表默认只显示未归档记录。
...
1. 云端部署时，打开 `/login`，二维码会自动使用云端访问地址。
2. 局域网演示时，运行 `npm run dev` 后，把登录页二维码地址改成 `http://电脑局域网IP:3000/login`。
3. 手机和电脑需要在同一个 Wi-Fi 下，手机扫码即可打开登录页。
...
部署成功后，请打开公网 `/login` 页面，页面右侧会自动生成对应公网地址的二维码。
...
src/app/login/page.tsx     统一登录与二维码入口
